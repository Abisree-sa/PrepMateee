...
export async function evaluateStudentCode(
  ...
  // If customInput is provided, run single testcase evaluation
  ...
  // Check for obvious syntax / compilation errors
  ...
  // Execute JavaScript natively using Node vm context
  if (lang === 'javascript' || lang === 'js') {
      ...
      try {
        ...
        const script = new vm.Script(`
          ...
          try {
            ...
            } else if (typeof numIslands === 'function') {
              const lines = input.trim().split('\\n');
              const grid = lines.slice(1).map(l => l.trim().split(''));
              result = String(numIslands(grid));
            } else if (typeof coinChange === 'function') {
              const lines = input.trim().split('\\n');
              const coins = lines[1].trim().split(/\\s+/).map(Number);
              const amount = Number(lines[2].trim());
              result = String(coinChange(coins, amount));
            } else if (typeof solve === 'function') {
              result = String(solve(input));
            } else {
              result = "Solution function executed";
            }
          } catch(e) {
    ...
    // For Java, Python, C++, C: Run Intelligent Syntax & Output Simulation Engine
    for (const tc of targetCases) {
